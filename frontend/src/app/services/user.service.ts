...
export class UserService {
  ...
  public userObservble:Observable<User>;

  constructor(private http:HttpClient, private toastrService:ToastrService) { 
    this.userObservble=this.userSubject.asObservable();
  }
      ...
      tap({
        next: (user) => {
          ...
          this.toastrService.success(
            `Welcome to DoubleOut ${user.name}!`, 'Login Succesful'
          )
        },
        error: (errorResponse) => {
  ...
  register(userRegister: IUserRegister): Observable<User>{
    return this.http.post<User>(USER_REGISTER_URL, userRegister).pipe(tap({
      next: (user) => {
        ...
        this.userSubject.next(user);
        this.toastrService.success(
          `Welcome to the DoubleOut ${user.name}`,
          'Register Successful'
        )
      },
      error: (errorResponse) => {
